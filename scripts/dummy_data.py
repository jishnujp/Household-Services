...
from application.models import db, User, Role, Service, ProfessionalDetails
import requests
...
for user in users:
    ...
    file_name = download_image(
        user["profile_pic"],
        app.config["STATIC_FOLDER"] + "/storage/profile_pics",
        user["username"].split("@")[0] + ".jpg",
    )
    user["profile_pic"] = "storage/profile_pics/" + file_name
    ...
    file_name = download_pdf(
        ...
        app.config["STATIC_FOLDER"] + "/storage/documents",
        user["username"].split("@")[0] + ".pdf",
    )
    user["professional_details"]["document"] = "storage/documents/" + file_name
